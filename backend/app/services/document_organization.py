...
def coerce_section_label(
    ...
    inferred = infer_section_label(filename, summary, raw_text)
    normalized = (suggested or "").strip()
    if normalized not in SECTION_NAMES:
        return inferred
    if inferred != "General" and inferred != normalized:
        return inferred
...
async def organize_documents_with_ai(
    ...
    system_prompt = (
        ...
        "- sort_order is a zero-based integer for the case-wide review order."
    )
